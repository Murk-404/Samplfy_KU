...
var utils = require('@spotifly/utils');
var fetch = require('node-fetch');
var tinyparse = require('@eegli/tinyparse');
...
const {
  parse,
  help
} = tinyparse.createParser(defaultConfig, {
  options: {
    clientId: {
      required: true,
...
const authorize = async options => {
  ...
  const token = await fetch__default["default"]('https://accounts.spotify.com/api/token', {
    method: 'POST',
    headers: {
  ...
  if (!config.noEmit) {
    const outDir = await utils.writeJSON({
      path: config.outDir,
